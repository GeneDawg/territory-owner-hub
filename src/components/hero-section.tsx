...
export function HeroSection() {
  return (
    <Card className="w-full min-h-[600px] lg:h-[750px] bg-black/[0.96] relative overflow-hidden border-0 rounded-none">
      <Spotlight className="-top-40 left-0 md:left-60 md:-top-20" fill={NEON} />

      <div className="flex flex-col lg:flex-row h-full">
        {/* Left content - Text */}
        <motion.div
          className="flex-1 p-8 lg:p-16 relative z-10 flex flex-col justify-center"
          initial={{ opacity: 0, x: -40 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
        >
          <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full border w-fit"
          ...
          >
            ...
            <span className="text-sm" style={{ color: `${NEON}cc` }}>Territory Owner Hub</span>
          </div>
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight leading-[1.1]">
            ...
            <span className="block mt-2 bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-neutral-500 text-3xl md:text-4xl lg:text-5xl font-bold">
              Growth Engine
            </span>
          </h1>
          ...
          <p className="mt-4 text-neutral-500 max-w-xl text-sm md:text-base leading-relaxed">
          ...
          <a
            href="#flywheel"
            className="mt-8 inline-flex items-center gap-2 px-6 py-3 font-semibold rounded-lg transition-all duration-200 w-fit text-white hover:brightness-125"
            style={{
